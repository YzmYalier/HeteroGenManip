...
def furthest_point_sampling_with_segment_id(points, colors=None, n_samples=2048, segment_id=None):
    """
    points: [N, 3] tensor containing the whole point cloud
    n_samples: samples you want in the sampled point cloud typically &lt;&lt; N 
    """
    ...
    if segment_id is not None:
        segment_id = torch.Tensor(segment_id).cuda()
    ...
    for i in range(1, n_samples):
        ...
        sample_inds[i] = selected
    if colors is None:
        return points[sample_inds].cpu().numpy(), segment_id[sample_inds].cpu().numpy()
    else:
        return points[sample_inds].cpu().numpy(), colors[sample_inds].cpu().numpy(), segment_id[sample_inds].cpu().numpy()
