...
class StoreTops_Env(BaseEnv):
    def __init__(
        ...
        # load GAM Model
        self.model_GAM = GAM_Encapsulation(catogory="Tops_NoSleeve")
...
def StoreTops(pos, ori, usd_path, env_dx, env_dy, env_dz, theta, ground_material_usd, data_collection_flag, record_video_flag):

    env = StoreTops_Env(pos, ori, usd_path, env_dx, env_dy, env_dz, theta, ground_material_usd, record_video_flag)
    ...
    manipulation_points, indices, points_similarity = env.model_GAM.get_manipulation_points(input_pcd=env.garment_pcd, index_list=[1021, 1819])
    ...
    if data_collection_flag:
        for i in range(20):
            env.step()
        env.record(task_name="Store_Tops", stage_str="all")
    ...
    left_lift_points, right_lift_points = np.array([-0.35*sin_theta-dis*cos_theta+env.bag_center[0], env.bag_center[1]-0.35*cos_theta+dis*sin_theta, 0.5]), np.array([-0.25*sin_theta+dis*cos_theta+env.bag_center[0], env.bag_center[1]-0.25*cos_theta-dis*sin_theta, 0.5])    # set z-axis to 0.65 to make sure dexhand can lift the garment
    ...
    left_lift_points, right_lift_points = np.array([-0.35*sin_theta-dis*cos_theta+env.bag_center[0], env.bag_center[1]-0.35*cos_theta+dis*sin_theta, lift_height]), np.array([-0.25*sin_theta+dis*cos_theta+env.bag_center[0], env.bag_center[1]-0.25*cos_theta-dis*sin_theta, lift_height])    # set z-axis to 0.65 to make sure dexhand can lift the garment
    ...
    if dis > 0.22:
        dis = 0.22
    ylen = 0.01
    left_lift_points, right_lift_points = np.array([ylen*sin_theta-dis*cos_theta+env.bag_center[0], env.bag_center[1]+ylen*cos_theta+dis*sin_theta, lift_height]), np.array([ylen*sin_theta+dis*cos_theta+env.bag_center[0], env.bag_center[1]+ylen*cos_theta-dis*sin_theta, lift_height])  
    # move both dexhand to the lift points
    env.bimanual_dex.dense_move_both_ik(left_pos=left_lift_points, left_ori=np.array([0.579, -0.579, -0.406, 0.406]), right_pos=right_lift_points, right_ori=np.array([0.406, -0.406, -0.579, 0.579]))
    ...
    left_lift_points, right_lift_points = np.array([ylen*sin_theta-dis*cos_theta+env.bag_center[0], env.bag_center[1]+ylen*cos_theta+dis*sin_theta, 0.3]), np.array([ylen*sin_theta+dis*cos_theta+env.bag_center[0], env.bag_center[1]+ylen*cos_theta-dis*sin_theta, 0.3])  
    # move both dexhand to the lift points
    env.bimanual_dex.dense_move_both_ik(left_pos=left_lift_points, left_ori=np.array([0.579, -0.579, -0.406, 0.406]), right_pos=right_lift_points, right_ori=np.array([0.406, -0.406, -0.579, 0.579]))
    
     # open hand to release the garment
...
if __name__=="__main__":
    ...
    StoreTops(pos, ori, usd_path, env_dx, env_dy, env_dz, theta, args.ground_material_usd, args.data_collection_flag, args.record_video_flag)
