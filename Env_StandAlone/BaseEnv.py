...
class BaseEnv:
    ...
    def record(self, task_name: str, stage_str: str = "all"):
    ...
    def stop_record(self):
        if self.record_flag == True:
            ...
            self.saving_data_replay["stage_all"] = np.array(
                self.saving_data
            )
            self.saving_data_replay["stage_remain"] = np.array(
                self.saving_data[self.data_split :]
            )
            print("Data length: ", len(self.saving_data))
            print("Data split index: ", self.data_split)
            print("Remain Data length: ", len(self.saving_data[self.data_split :]))
            # clear record data
            self.saving_data = []
    ...
    def replay(self, stage_str):
        # load data
        self.data = self.saving_data_replay[f"stage_{stage_str}"]
        # current timestep
        self.time_ptr = 0
