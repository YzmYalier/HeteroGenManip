...
def StackTops_Validation(active_pos, passive_pos, active_ori, passive_ori, active_usd_path, passive_usd_path, ground_material_usd, validation_flag, record_video_flag, training_data_num, checkpoint_num, stage_str, policy_name):
    ...
    if record_video_flag:
        if not os.path.exists(f"Data/Stack_Tops_ood_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/video"):
            os.makedirs(f"Data/Stack_Tops_ood_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/video")
        env.env_camera.create_mp4(get_unique_filename(f"Data/Stack_Tops_ood_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/video/video", ".mp4"))
    try:
    ...
    if validation_flag:
        if not os.path.exists(f"Data/Stack_Tops_ood_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}"):
            os.makedirs(f"Data/Stack_Tops_ood_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}")
        # write into .log file
        with open(f"Data/Stack_Tops_ood_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/validation_log.txt", "a") as f:
        ...
        if not os.path.exists(f"Data/Stack_Tops_ood_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/final_state_pic"):
            os.makedirs(f"Data/Stack_Tops_ood_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/final_state_pic")
        env.env_camera.get_rgb_graph(save_or_not=True,save_path=get_unique_filename(f"Data/Stack_Tops_ood_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/final_state_pic/img",".png"))
...
if __name__=="__main__":
    ...
    if args.env_random_flag or args.garment_random_flag:
        ...
        if args.garment_random_flag:
            ...
            assets_lists = os.path.join(Base_dir,"FoundationModels/GAM/checkpoints/Tops_NoSleeve/assets_list.txt")
            assets_list = []
