...
def Store_Mug_Validation(active_pos, passive_pos, active_ori, passive_ori, active_usd_path, rigid_scale, ground_material_usd, validation_flag, record_video_flag, training_data_num, checkpoint_num, stage_str, policy_name):
    ...
    if policy_name != "TriDFA":
        ...
        for i in range(step_lim):
            ...
            print(env.franka.get_cur_ee_pos())
            print(action)
            
            env.franka.Dense_Rmpflow_Move_val(
            ...
            gripper_state = round(action[7])
            cur_gripper_state = env.franka.gripper_state
    ...
    success = rigid_flag
    if record_video_flag:
        if not os.path.exists(f"Data/Store_Mug_ind_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/video"):
            os.makedirs(f"Data/Store_Mug_ind_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/video")
        env.env_camera.create_mp4(get_unique_filename(f"Data/Store_Mug_ind_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/video/video", ".mp4"))
    if validation_flag:
        if not os.path.exists(f"Data/Store_Mug_ind_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/final_state_pic"):
            os.makedirs(f"Data/Store_Mug_ind_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/final_state_pic")
        env.env_camera.get_rgb_graph(save_or_not=True,save_path=get_unique_filename(f"Data/Store_Mug_ind_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/final_state_pic/img",".png"))
        with open(f"Data/Store_Mug_ind_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/validation_log.txt", "a") as f:
            f.write(f"result:{success} \n")
        with open(f"Data/Store_Mug_ind_Validation_{policy_name}_{stage_str}_{checkpoint_num}_{training_data_num}/pose_log.txt", "a") as f:
